...
namespace Flare
{
    namespace SceneGraph
    {
        ...
        void IndirectRenderedSceneGraph::render()
        {
            ...
            auto commandGroupRanges = getCommandGroupRanges(sortedDrawCommands);
        ...
        void IndirectRenderedSceneGraph::sortDrawCommandsByMaterial(SortableDrawCommands &unsortedDrawCommands)
        {
            std::sort(
        ...
        IndirectRenderedSceneGraph::CommandGroupRanges IndirectRenderedSceneGraph::getCommandGroupRanges(const SortableDrawCommands &drawCommandsSortedByMaterial)
        {
            auto lastCommandGroupIndexStart = size_t{0};
            auto commandGroupRanges = CommandGroupRanges{};

            if (drawCommandsSortedByMaterial.size() == 1) {
                commandGroupRanges.emplace_back(lastCommandGroupIndexStart, 0);
                return commandGroupRanges;
            }
            ...
            for (size_t i = 1; i < drawCommandsSortedByMaterial.size(); ++i) {
                ...
                if ((currentDrawCommand.shaderData.hashedAlias != startOfRangeDrawCommand.shaderData.hashedAlias)
                    ...
                    if (isLastIteration) {
                        commandGroupRanges.emplace_back(lastCommandGroupIndexStart, i);
                    } else {
                        commandGroupRanges.emplace_back(lastCommandGroupIndexStart, i - 1);
                    }
        ...
        IndirectRenderedSceneGraph::CombinedRenderDataBuffers IndirectRenderedSceneGraph::getCombinedRenderDataBuffers(SortableDrawCommands &sortedDrawCommands, const CommandGroupRanges &commandGroupRanges)
        {
